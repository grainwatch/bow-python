...
label_to_id = {'negative': -1, 'corn': 1, 'rye': 2, 'triticale': 3, 'wheat': 4, 'half_corn': 5, 'half_rye': 6, 'half_triticale': 7, 'half_wheat': 8}
...
def load_objects(imgs_pathname: str, labels_pathname: str):
    imgspath, labelspath = Path(imgs_pathname), Path(labels_pathname)
...
def create_data_from_classes(imgfolder_pathname: str, labelfolder_pathname: str, dst_pathname: str, class_list: List[str], test_amount: float=0, scale: float=1.0, window_size: Tuple[int, int]=(128, 128)):
    ...
    for label_classdir in labelpaths:
        ...
        if img_classdir.exists():
            samples = _create_class_samples_2(img_classdir, label_classdir, scale, window_size)
            negative_samples += _create_negative_samples_from_class_2(img_classdir, label_classdir, scale, window_size)
            train_samples, test_samples = _split_samples(samples, test_amount)
            class_train_path = train_path.joinpath(label_classdir.name)
            class_train_path.mkdir(exist_ok=True)
            class_test_path = test_path.joinpath(label_classdir.name)
...
def _create_class_samples_2(classimg_path: Path, labelfolder_path: Path, scale: float, window_size: Tuple[int, int]) -> List[np.ndarray]:
    samples = []
    rect_dict = _load_labels(list(labelfolder_path.glob('*.json')))
    imgpaths = list(classimg_path.iterdir())
    imgpaths = list(filter(lambda imgpath: _name_from_path(imgpath) in rect_dict, imgpaths))
    img_rects = list(map(lambda imgpath: rect_dict[_name_from_path(imgpath)], imgpaths))
    imgs = _load_imgs_from_path(imgpaths, scale)
    for img, rects in zip(imgs, img_rects):
        rects = map(lambda rect: rect.rescale(scale), rects)
        rects = map(lambda rect: rect.to_window(window_size), rects)
        rects = map(lambda rect: rect.move_to_shape(img.shape), rects)
        samples += list(map(lambda rect: rect.to_roi(img), rects))
    return samples
...
def _create_negative_samples_from_class(classimg_path: Path, labelfolder_path: Path, scale: float, window_size: Tuple[int, int]) -> List[np.ndarray]:
    ...
    imgs = _load_imgs_from_path(imgpaths, scale)
    for img, label in zip(imgs, labels):
        samples += _negative_samples_from_img(img, label, window_size)
    return samples

STRIDE = (192, 192)

def _create_negative_samples_from_class_2(classimg_path: Path, labelfolder_path: Path, scale: float, window_size: Tuple[int, int]) -> List[np.ndarray]:
    samples = []
    rect_dict = _load_labels(list(labelfolder_path.glob('*.json')), False)
    imgpaths = list(classimg_path.iterdir())
    imgpaths = list(filter(lambda imgpath: _name_from_path(imgpath) in rect_dict, imgpaths))
    img_rects = list(map(lambda imgpath: rect_dict[_name_from_path(imgpath)], imgpaths))
    imgs = _load_imgs_from_path(imgpaths, scale)
    for img, rects in zip(imgs, img_rects):
        rects = list(map(lambda rect: rect.rescale(scale), rects))
        samples += _negative_samples_from_img_2(img, rects, window_size, STRIDE)
    return samples
...
def _load_labels(labelpaths: List[Path], ignore_difficult: bool=True) -> Dict[str, List[rectangle.Rect]]:
    rect_dict = {}
    for labelpath in labelpaths:
        labelname = _name_from_path(labelpath)
        rects, labels = rectangle.rects_labels_from_json(labelpath)
        if ignore_difficult:
            rects, labels = zip(*filter(lambda rect_label: label_to_id[rect_label[1]] < 5, zip(rects, labels)))
        rect_dict[labelname] = list(rects)
    return rect_dict
...
def _negative_samples_from_img_2(img: np.ndarray, rois: List[rectangle.Rect]=None, window_size: Tuple[int, int]=(128, 128), stride: Tuple[int, int]=(192, 192)):
    roi_img_list = []
    width, height = window_size
    stride_x, stride_y = stride
    x, y = 0, 0
    while y + height < img.shape[0]:
        if x + width > img.shape[1]:
            ...
            y += stride_y
            continue #skip loop iteration to check y condition
        if not is_rect_colliding(rectangle.Rect(x, y, width, height), rois, img):
            roi_img = img[y:y+height, x:x+width]
            roi_img_list.append(roi_img)
        x += stride_x
    return roi_img_list
...
def is_rect_colliding(rect_to_check: rectangle.Rect, rects: List[rectangle.Rect], img: np.ndarray=None):
    x_check, y_check, width_check, height_check = dataclasses.astuple(rect_to_check)
    #visual = cv.rectangle(img, (x_check, y_check), (x_check+width_check, y_check+height_check), (255, 0, 0))
    for rect in rects:
        x, y, width, height = dataclasses.astuple(rect)
...
if __name__ == '__main__':
    ...
    #scale = 128 / size
    #print(scale)
    create_data_from_classes('C:/Users/Alex/Desktop/Bilder_Korner_original_20180411', 'C:/Users/Alex/Desktop/UNI', 'C:/Users/Alex/Desktop/all256', ['corn', 'rye', 'triticale', 'wheat'], 0.2, 0.7, (256, 256))
